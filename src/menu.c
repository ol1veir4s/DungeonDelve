...
static const char *multiplayerOptions[MAX_OPTIONS] = {
    "HOST A SERVER",
    "CONNECT",
    "RETURN",
    "EXIT"
...
MenuData* menu_screen(void) {
    ...
    while (1) {
        ...
        switch (menuData->currentState) {
            case MENU_MAIN:
                DrawMainMenu(menuData->selectedOption);
                break;
            case MENU_DIFFICULTY:
                DrawDifficultyMenu(menuData->selectedOption);
                break;
            case MENU_WORLD_SETTINGS:
                DrawWorldSettings(menuData->selectedOption);
                break;
            case MENU_MULTIPLAYER:
                DrawMultiplayerMenu(menuData->selectedOption);
                break;
            case MENU_HOSTING:
        ...
        DrawCircleGradient(menuData->verticalCenter + 300, 0, 2 * SCREEN_WIDTH, Fade(menuData->backgroundColor, 0.0f), Fade(menuData->backgroundColor, 1.0f));
        EndDrawing();
...
void DrawBackground(Texture2D logoTexture, Texture2D wallTexture, Texture2D fireTexture, Texture2D rainTexture) {
    ...
    DrawCircleGradient(menuData->verticalCenter + 300, 500, 1000, Fade(WHITE, 0.0f), Fade(WHITE, menuData->RainingAlpha / 3.0f));
}
...
void DrawDifficultyMenu(int selectedOption) {
    ...
    DrawText("SELECT YOUR DIFFICULTY", SCREEN_WIDTH / 2 - 200 - 70, menuData->verticalCenter - 100, 40, WHITE);
}
...
void DrawMultiplayerMenu(int selectedOption) {
    for (int i = 0; i < MAX_OPTIONS; i++) {
        ...
        Rectangle optionRect = {rectPos.x, rectPos.y, 400, 40};
        Color color = (i == selectedOption) ? RED : ColorFromNormalized((Vector4){0.44f, 0.44f, 0.44f, 1.0f});
        DrawOption(multiplayerOptions[i], optionRect, color);
    }
}
